...
function getBizId() {
  ...
  return new URLSearchParams(window.location.search).get("biz") || "detroit-plumbing-co";
}
...
export default function CommandCenter() {
  ...
  async function resolveTicket(id) {
    ...
    try {
      const res = await fetch(`${API_BASE}/tickets/${id}/resolve`, { method: "POST" });
  ...
  const bizInitials = bizId.slice(0, 2).toUpperCase();
      ...
      <aside className="hidden w-[210px] flex-shrink-0 flex-col bg-[#111111] lg:flex">
        ...
        <div className="px-5 py-5 space-y-3">
          ...
          <div>
            ...
            <p className="mt-0.5 break-all text-[11px] font-semibold text-[#555]">{bizId}</p>
          </div>
        </div>
      </aside>
                    ...
                    <thead>
                      <tr className="bg-[#0f0f0f]">
                        {["Ticket", "Time", "Customer", "Issue", "Urgency", "Status", "Action"].map((h) => (
                          <th key={h} className="whitespace-nowrap px-5 py-3 text-[9px] font-black uppercase tracking-[0.24em] text-[#333]">
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {displayed.map((t, i) => {
                        ...
                        return (
                          <tr
                            key={id}
                            className={`transition-all duration-300 hover:bg-[#161616] ${
                              i !== displayed.length - 1 ? "border-b border-[#161616]" : ""
                            } ${isRes ? "opacity-0 scale-[0.98]" : "opacity-100"}`}
                          >
                            <td className="px-5 py-3.5">
                              <span className="font-mono text-xs font-bold text-[#444]">{formatId(id)}</span>
                            </td>
                            <td className="whitespace-nowrap px-5 py-3.5">
                              <span className="text-xs text-[#444]">{timeAgo(t.created_at)}</span>
                            </td>
                            <td className="px-5 py-3.5">
                            ...
                            <td className="max-w-[200px] px-5 py-3.5">
                              <p className="line-clamp-2 text-xs font-semibold leading-snug text-[#ccc]">
                                {t.issue_summary || t.raw_message || "—"}
                              </p>
                              {t.ticket_type && (
                                <p className="mt-0.5 text-[10px] text-[#444]">{t.ticket_type}</p>
                              )}
                            </td>
                            <td className="whitespace-nowrap px-5 py-3.5">
                              <UrgencyCell urgency={t.urgency} />
                            </td>
                            <td className="whitespace-nowrap px-5 py-3.5">
                              ...
                              {!isDone ? (
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <button
                                    ...
                                    disabled={isRes}
                                    className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[10px] font-bold transition-colors disabled:opacity-40 ${
                                      t.urgency === "emergency"
                                        ? "bg-red-500/12 text-red-400 hover:bg-red-500/20"
                                        : "bg-[#f97316]/12 text-[#f97316] hover:bg-[#f97316]/20"
                                    }`}
                                  >
                                    {isRes ? <Spinner /> : null}
                                    Resolve
